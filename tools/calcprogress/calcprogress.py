...
def exec_progress_callback(src_code: int, total_code: int, src_data: int,
                           total_data: int):
    ...
    bytes_per_trophy = total_code / 120
    bytes_per_event = total_data / 37

    trophy_count = floor(src_code / bytes_per_trophy)
    event_count = floor(src_data / bytes_per_event)

    bytes_to_go_next_trophy = (
        (trophy_count + 1) * bytes_per_trophy) - src_code

    print(
        "You have collected {} of 120 energy spheres and completed {} of 37 stages."
        .format(trophy_count, event_count))
    print("Code bytes to go for next energy sphere:",
          floor(bytes_to_go_next_trophy) + 1)
...
def slice_group_progress_callback(name: str, src_size: int, total_size: int):
    ...
    print(
        f"{slice_progress}% of this slice group ({name}) has been decompiled")
