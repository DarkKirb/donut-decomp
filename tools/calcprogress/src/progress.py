...
def calc_exec_progress(dol: Dol, asm_list: List[AsmUtil.Section],
                       callback: "function", correction: int):
    ...
    # Decompiled sizes
    src_code = total_code - asm_code - correction - 292828
    src_data = total_data - asm_data - 183108
    # Percentages
    code_percent = src_code / total_code
    data_percent = src_data / total_data
    print("Progress:")
    print(
        f"\tCode sections: {src_code} / {total_code} bytes in src ({code_percent:%})"
    )
    print(
        f"\tData sections: {src_data} / {total_data} bytes in src ({data_percent:%})"
    )
...
def calc_slice_group_progress(group: SliceGroup,
                              asm_list: List[AsmUtil.Section],
                              callback: "function"):
